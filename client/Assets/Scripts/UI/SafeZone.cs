...
public class SafeZone : MonoBehaviour
{
    ...
    float visualCorrectionPercentage = 4f;
    ...
    IEnumerator ShowDangerZone()
    {
        ...
        if (currentValue >= 0.85f)
            yield break;
        currentValue += 0.05f;
        parameterValue = Mathf.Round(currentValue * 100) / 100f;
        dangerZone.sharedMaterial.SetFloat("_AlphaMultiplier", parameterValue);

        dangerZoneRing.sharedMaterial.SetFloat("_AlphaMultiplier", parameterValue);
    }
    ...
    void Update()
    {
        ...
        radius -= ((radius * visualCorrectionPercentage) / 100);
        ...
        float value = (float)(
            finalVfxValue + (currentRadius) * (-finalVfxValue) / (initialPlayableRadius)
        );

        dangerZone.sharedMaterial.SetFloat("_Progress", value);
        dangerZoneRing.transform.localScale = new Vector3(
            1 - value,
            dangerZoneRing.transform.localScale.y,
            1 - value
        );
    }
}
